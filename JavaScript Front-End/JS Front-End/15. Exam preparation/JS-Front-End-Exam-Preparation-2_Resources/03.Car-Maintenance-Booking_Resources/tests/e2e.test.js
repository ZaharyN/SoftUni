...
const endpoints = {
  catalog: '/jsonstore/appointments',
  byId: (id) => `/jsonstore/appointments/${id}`,
};
...
describe('E2E tests', function () {
  ...
  describe('Car Maintenance Booking Tests', () => {
    it('Load Appointment', async () => {
      const data = mockData.list;
      const { get } = await handle(endpoints.catalog);
      get(data);

      await page.goto(host);
      await page.waitForSelector('#load-appointments');

      await page.click('#load-appointments');

      const list = await page.$$eval(`#appointments #appointments-list .appointment`, (t) =>
   ...
   it('Add Appointment', async () => {
      ...
      await page.fill('#car-model', data.model);
      await page.selectOption('#car-service', data.service);
      ...
      expect(postData.service).to.equal(data.service);
      expect(postData.date).to.equal(data.date);

      const [model] = await page.$$eval(`#car-model`, (t) =>
        t.map((s) => s.value)
      );
      const [service] = await page.$$eval(`#car-service`, (t) =>
        t.map((s) => s.value)
      );
    ...
    it('Edit Appointment (Has Input)', async () => {
      await page.goto(host);
      const data = mockData.list[0];

      await page.click('#load-appointments');
      await page.waitForSelector('#appointments-list');
      await page.click('#appointments-list .appointment .change-btn');

      const allCourse = await page.$$eval(`#form input`, (t) =>
        t.map((s) => s.value)
      );
      const serviceValue = await page.$eval('#form select#car-service', el => el.value);

    
      expect(allCourse[0]).to.include(data.model);
      expect(serviceValue).to.include(data.service);
      expect(allCourse[1]).to.include(data.date);

    });

    it('Edit Appointment (Makes API Call)', async () => {
      const data = mockData.list[0];
      await page.goto(host);
      ...
      await page.waitForSelector('#appointments-list');
      await page.click('#appointments-list .appointment .change-btn');
      await page.fill('#car-model', data.model + '2');

      const [request] = await Promise.all([
        onRequest(),
        page.click('#edit-appointment'),
      ]);

      const postData = JSON.parse(request.postData());
      expect(postData.model).to.equal(data.model + '2');
    });
    ...
    it('Delete Appointment', async () => {
      ...
      await Promise.all([
        onResponse(),
        page.click(
          `#appointments-list .appointment .delete-btn`
        ),
      ]);
